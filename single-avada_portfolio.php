<?php get_header(); ?>
	...
	<div id="content" class="full-width portfolio-<?php echo $portfolio_width; ?>">
		...
		<div class="single-navigation clearfix">
			<?php
			if($portfolioID) {
				$previous_post_link = previous_post_link_plus(array('format' => '%link', 'link' => __('Previous', 'Avada'), 'in_same_tax' => 'product_cat', 'in_cats' => $nav_categories, 'return' => 'href'));
			} else {
			...
			<?php 
			if($portfolioID) {
				$next_post_link = next_post_link_plus(array('format' => '%link', 'link' => __('Next', 'Avada'), 'in_same_tax' => 'product_cat', 'in_cats' => $nav_categories, 'return' => 'href'));
			} else {
		...
		<div id="post-<?php the_ID(); ?>" <?php post_class(); ?>>
			...
			<div class="project-content">
				...
				<div class="project-info">
					...
					<?php endif; ?>
					<?php if(get_the_term_list($post->ID, 'product_cat', '', '<br />', '')): ?>
					<div class="project-info-box">
						<h4><?php echo $data['categories_title']; ?>:</h4>
						<div class="project-terms">
							<?php echo get_the_term_list($post->ID, 'product_cat', '', '<br />', ''); ?>
						</div>
					</div>
					<?php endif; ?>
